...
public class LookupService {
  ...
  "BL","MF", "BQ", "SS", "O1" };
    ...
    public Country getCountryV6(final InetAddress addr) {
        ...
        final int ret = seekCountryV6(addr) - COUNTRY_BEGIN;
        if (ret == 0) {
            return UNKNOWN_COUNTRY;
        }
        else {
    ...
    public synchronized DatabaseInfo getDatabaseInfo() {
        ...
        try {
            ...
            for (int i=0; i<STRUCTURE_INFO_MAX_SIZE; i++) {
                final int read = file.read( delim );
    ...
    synchronized void _check_mtime(){
      try {
        if ((dboptions & GEOIP_CHECK_CACHE) != 0){
          final long t = databaseFile.lastModified();
          if (t != mtime){
    ...
    public Location getLocationV6(final String str) {
        if (dnsService == 0) {
            ...
            final String str2 = getDnsAttributes(str);
    ...
    String getDnsAttributes(final String ip) {
	    ...
	    // TODO don't specify ws1, instead use ns servers for s.maxmind.com
            ...
            final Attributes attrs = ictx.getAttributes(licenseKey + "." + ip + ".s.maxmind.com", new String[] {"txt"});
    ...
    public Location getLocationwithdnsservice(final String str) {
	    ...
	    key = st.nextToken();
	        ...
	        value = "";}
	    if (key.equals("co")) {
		final Integer i = (Integer)hashmapcountryCodetoindex.get(value);
    ...
    public synchronized Location getLocationV6(final InetAddress addr) {
        final int record_pointer;
        final byte[] record_buf = new byte[FULL_RECORD_LENGTH];
        int record_buf_offset = 0;
        final Location record = new Location();
        int str_length = 0;
        int j;
	    final int seek_country;
    ...
    public synchronized String getOrg(final long ipnum) {
        ...
        final int record_pointer;
        int str_length = 0;
        final byte [] buf = new byte[MAX_ORG_RECORD_LENGTH];
        final String org_buf;
